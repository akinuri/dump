...
def cv_threshold(img, thresh=240, maxval=255):
    gray     = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    threshed = cv2.threshold(gray, thresh, maxval, cv2.THRESH_BINARY_INV)[1]
    return threshed

def get_contours(img, to_gray=None):
...
orig_threshed = cv_threshold(orig_img)
orig_contours = get_contours(orig_threshed)
...
dilated_contours = get_contours(dilated_mask, True)
dilated_output   = draw_contours(orig_img, dilated_contours)

# cv2.imshow("orig_img", orig_img)
# cv2.imshow("orig_threshed", orig_threshed)
